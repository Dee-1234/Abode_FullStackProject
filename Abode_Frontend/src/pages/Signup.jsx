...
const Signup = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            navigate('/login', { state: { message: 'Account created successfully! Sign in below.' } });
        } catch (err) {
            console.error("Signup error:", err);
            setError(err.response?.data?.message || err.response?.data || 'Failed to initialize account.');
        } finally {
            setLoading(false);
        }
    };
        ...
        <div className="auth-wrapper">
            ...
            >
                <div style={{ textAlign: 'center', marginBottom: '40px' }}>
                    ...
                    <h2 style={{ margin: 0, fontSize: '30px', fontWeight: '800', letterSpacing: '-0.02em' }}>Create Account</h2>
                    <p style={{ color: '#64748b', fontSize: '15px', marginTop: '8px' }}>Join the Abode neighborhood network</p>
                </div>
                ...
                <div className="card-footer">
                    <p style={{ color: '#64748b', fontSize: '14px', margin: 0 }}>
                        Already have an account? <Link to="/login" style={{ color: '#3b82f6', fontWeight: '700', textDecoration: 'none', marginLeft: '5px' }}>Sign in</Link>
                    </p>
                </div>
            </motion.div>
        </div>
